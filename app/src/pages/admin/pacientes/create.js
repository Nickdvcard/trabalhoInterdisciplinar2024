          ...
          <div className="row border-bottom">
            ...
            <form method="POST" id="formCadastro">
              ...
              <div className="mb-3">
                ...
                <input type="text" id="first_name" name="first_name" className="form-control" maxLength="45" value={paciente.first_name} onChange={handleChange} required />
              </div>

              {/* Último Nome */}
              <div className="mb-3">
                <label htmlFor="last_name" className="form-label">Último Nome:</label>
                <input type="text" id="last_name" name="last_name" className="form-control" maxLength="45" value={paciente.last_name} onChange={handleChange} required />
              </div>

              {/* Observações */}
              <div className="mb-3">
                <label htmlFor="observations" className="form-label">Observações:</label>
                <textarea id="observations" name="observations" className="form-control" placeholder="Observações opcionais" maxLength="200" value={paciente.observations} onChange={handleChange}></textarea>
              </div>

              {/* Profissionais Pedagogos */}
              <div className="mb-3">
                <label htmlFor="teachers" className="form-label">Profissionais Pedagogos:</label>
                <select id="teachers" name="teacher" className="form-select" value={paciente.teacher} onChange={handleChange} required>
                  <option value="" disabled>-- Selecione --</option>
                  {teachers.map((teacher) => (
                    <option key={teacher.idProfissionais} value={teacher.idProfissionais}>{teacher.primeiroNome} {teacher.ultimoNome}</option>
                  ))}
                </select>
              </div>
              ...
              <div className="mb-3">
                ...
                <select id="conditions" name="condition" className="form-select" value={paciente.condition} onChange={handleChange} required>
                  <option value="" disabled>-- Selecione --</option>
                  {conditions.map((condition) => (
                    <option key={condition.idCondicoes} value={condition.idCondicoes}>{condition.nome}</option>
                  ))}
                </select>
              </div>
              ...
              <div className="mb-3">
                ...
                <input type="text" id="logradouro" name="logradouro" className="form-control" value={paciente.logradouro} onChange={handleChange} maxLength="45" required />
              </div>

              {/* Número */}
              <div className="mb-3">
                <label htmlFor="numero" className="form-label">Número:</label>
                <input type="number" id="numero" name="numero" className="form-control" value={paciente.numero} onChange={handleChange} required />
              </div>

              {/* Complemento */}
              <div className="mb-3">
                <label htmlFor="complemento" className="form-label">Complemento:</label>
                <input type="text" id="complemento" name="complemento" className="form-control" value={paciente.complemento} onChange={handleChange} maxLength="200" placeholder="Opcional" />
              </div>

              {/* CEP */}
              <div className="mb-3">
                <label htmlFor="cep" className="form-label">CEP:</label>
                <input type="text" id="cep" name="cep" className="form-control" value={paciente.cep} onChange={handleChange} pattern="\d{5}-\d{3}" title="Formato: 12345-678" placeholder="Formato: 12345-678" maxLength="9"/>
              </div>

              {/* UF */}
              <div className="mb-3">
                <label htmlFor="uf" className="form-label">Estado (UF):</label>
                <select id="uf" name="uf" className="form-select" value={paciente.uf} onChange={handleUfChange} required>
                  <option value="" disabled>-- Selecione --</option>
                  {ufs.map((uf) => (
              ...
              <div className="mb-3">
                ...
                <select id="cidade" name="cidade" className="form-select" value={paciente.cidade} onChange={handleCityChange} required disabled={!paciente.uf}>
                  <option value="" disabled>-- Selecione --</option>
                  {cidadesFiltradas.map((city) => (
                    <option key={city.idCidades} value={city.idCidades}>{city.nome}</option>
                  ))}
                </select>
              </div>
              ...
              <div className="mb-3">
                ...
                <select id="bairro" name="bairro" className="form-select" value={paciente.bairro} onChange={handleChange} required disabled={!paciente.cidade}>
                  <option value="" disabled>-- Selecione --</option>
                  {bairrosFiltrados.map((neighborhood) => (
                    <option key={neighborhood.idBairros} value={neighborhood.idBairros}>{neighborhood.nome}</option>
                  ))}
                </select>
              </div>
              ...
              <div className="form-group">
                <button type="button" className="btn btn-primary" onClick={handleCreatePaciente}>Salvar Paciente</button>
